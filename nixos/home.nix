...
let
  wallpaperDir = "/home/serj/.config/wallpaper";
  ...
  selectWallpaper = pkgs.writeShellScriptBin "select-wallpaper" ''
    ...
    MODE=$(printf 'fill\nfit' | ${pkgs.wofi}/bin/wofi --dmenu --prompt "Mode" --insensitive)
    [ -z "$MODE" ] && MODE="fill"
...
in {
  ...
  wayland.windowManager.sway = {
    ...
    extraConfig = ''
      ...
      for_window [title="(?i)(save|сохранить|open|открыть|choose|выберите|look in|auntethication)"] floating enable
